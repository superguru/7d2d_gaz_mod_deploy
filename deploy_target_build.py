            ...
            try:
                ...
                if compile_result.returncode == 0:
                    ...
                    if run_result.returncode == 0:
                        ...
                        print(f"Warning: Failed to run version extractor: {run_result.stderr}")
                else:
                    print(f"Warning: Failed to compile version extractor: {compile_result.stderr}")
            finally:
...
def main():
    ...
    # Pre-extract ModName from sys.argv so it can be used in parser help text.
    # argparse will validate it properly during parse_args().
    _mod_name_preview = (
        sys.argv[1]
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=f"""
Examples:
          %(prog)s {MOD_NAME} Debug "bin/Debug" "D:/Projects/ModName"
  %(prog)s {MOD_NAME} Release "bin/Release" "C:/Source/ModSource"
  %(prog)s {MOD_NAME} Debug "bin/Debug" "D:/Projects/ModName" --clean
  %(prog)s {MOD_NAME} Debug "bin/Debug" "D:/Projects/ModName" --force
  %(prog)s {MOD_NAME} Debug "bin/Debug" "D:/Projects/ModName" --clean --force
  %(prog)s {MOD_NAME} Debug "bin/Debug" "D:/Projects/ModName" --no-pause
...
Use --no-pause to skip pausing on errors (default: pause on errors).
    ...
    # Set the global MOD_NAME from the parsed argument
    ...
    try:
        ...
        print("=" * 50)
        print(f"Command: {' '.join(f'{arg}' if i == 0 else f'\"{arg}\"' for i, arg in enumerate(sys.argv))}")
